...
enum { LyrBg, LyrBlur, LyrBottom, LyrTile, LyrFloat, LyrTop, LyrFS, LyrOverlay, LyrIMPopup, LyrBlock, NUM_LAYERS }; /* scene layers */
	...
	for (r = rules; r < END(rules); r++) {
		if ((!r->title || regex_match(r->title, title))
			...
			if (c->isfloating || !mon->lt[mon->sellt]->arrange) {
                ...
                newx = (int)round(r->x ? (r->x <= 1 ? (b.width - newwidth) * r->x + b.x : r->x + b.x) : c->geom.x);
				newy = (int)round(r->y ? (r->y <= 1 ? (b.height - newheight) * r->y + b.y : r->y + b.y) : c->geom.y);
				apply_resize = 1;
...
#endif
	if (corner_radius > 0) {
		c->round_border = wlr_scene_rect_create(c->scene, 0, 0, (float[])COLOR(colors[c->isurgent ? SchemeUrg : SchemeNorm][ColBorder]));
...
update_client_focus_decorations(Client *c, int focused, int urgent)
{
	if (corner_radius > 0 && c->round_border) {
		wlr_scene_rect_set_color(c->round_border, 
                urgent ? (float[])COLOR(colors[SchemeUrg][ColBorder]) 
                : (focused ? (float[])COLOR(colors[SchemeSel][ColBorder]) : (float[])COLOR(colors[SchemeUrg][ColBorder])));
	}
	if (shadow && c->shadow) {
